...
function renderizarDadosNaTela() {
    const frequenciaAlunosNaData = frequencias.find(item => item.data === dataParam);
    ...
    for (let a = 0; a < alunos.length; a++) {
        ...
        if (frequenciaAlunosNaData.frequenciaHoje[a].statusAluno === "Faltou") {
            checkbox.checked = false;
            paragrafo1.innerText = "Faltou";
        } 
        else if (frequenciaAlunosNaData.frequenciaHoje[a].statusAluno === "Falta Justificada") {
            checkbox.checked = false;
    ...
    const divFrequencia3 = document.querySelector(".frequencia-3");

    const divFrequencia31 = document.createElement("div");
    divFrequencia31.classList.add("frequencia-3-1");
    const linkCancelar = document.createElement("a");
    linkCancelar.href = `frequencia.html?data=${dataParam}`;
    linkCancelar.innerText = "Cancelar";
    divFrequencia31.appendChild(linkCancelar);

    const divFrequencia32 = document.createElement("div");
    divFrequencia32.classList.add("frequencia-3-2");
    const linkSalvar = document.createElement("a");
    linkSalvar.id = "enviar-frequencia";
    linkSalvar.href = `frequencia.html?data=${dataParam}`;
    linkSalvar.innerText = "Salvar";
    divFrequencia32.appendChild(linkSalvar);

    divFrequencia3.appendChild(divFrequencia31);
    divFrequencia3.appendChild(divFrequencia32);

    linkSalvar.addEventListener("click", antesEnvio);
}
