...
async function main() {
    ...
    const linkNovaFrequencia = document.querySelector('.conteudo-2-2-2-4 a');
    linkNovaFrequencia.addEventListener('click', adicionarParametrosAoLink);
}
...
async function carregarCalendario(mesSelecionado) {
    ...
    if (frequenciaCad === "nao") {
        const div1 = document.createElement("div");
        div1.classList.add("conteudo-2-2-2-3");
        const p1 = document.createElement("p");
        p1.innerText = "Atenção! Vamos começar? Clique em nova frequência para fazer o primeiro registro.";
        div1.appendChild(p1);

        const div2 = document.createElement("div");
        div2.classList.add("conteudo-2-2-2-4");
        const a2 = document.createElement("a");
        a2.innerText = "+ Nova Frequência";
        a2.href = "pagina-frequencia.html";
        div2.appendChild(a2);
        ...
        linkEditar.href = `frequencia-editar.html?data=${dataParam}`;
...
async function deletaDadosDoBackend() {
    const resultado = await fetch(`/api/frequencias/${dataParam.split("/")[0]}-${dataParam.split("/")[1]}-${dataParam.split("/")[2]}`, {
        method: 'DELETE',
        headers: {
...
async function adicionarParametrosAoLink(event) {
    ...
    if (dataParam) {
        linkNovaFrequencia.href = "pagina-frequencia.html?data=" + dataParam;
    }
}
